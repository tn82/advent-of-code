...
def input2():
    with open(os.path.join(day_path, "input.txt"), "r") as file:
        return [line.rstrip().replace("FJ", "-|").replace("L7", "-|") for line in file]
...
def part_two():
    ...
    grid_replace = {}
    for r, line in enumerate(test2()):
        for c, col in enumerate(line):
            grid_replace[(r, c)] = col
    ...
    for coo in grid:
        ...
        while coo_s:
            ...
            if coo_s not in grid:
                coo_s = None
            #elif coo_s in dists and grid_replace[coo_s] in ("|", "F", "J", "7", "L", "S"):
            #    count_crosses += 1
    ...
    printer(grid, dists, None)

    printer(grid_replace, dists, None)
    print("Part 2: ", sums)
    assert(sums == 6800)
